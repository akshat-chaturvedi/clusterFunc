...
class GaiaQuerier:
    def __init__(self, clusterName):
        ...
        job = Gaia.upload_table(upload_resource=f"gData/{self.clusterName}.csv", table_name=f"{self.clusterName}",
                                format="csv")

        job1 = Gaia.launch_job("SELECT TOP 100000 "
                               f"source_id, hip.{clusterName.lower()}_oid, parallax, parallax_error, gaia.ra, gaia.dec,"
        ...
        job1_results.write(f"GAIAData/{self.clusterName}_Results.csv", format="csv")
